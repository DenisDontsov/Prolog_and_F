...
let inp = new Label(Text = "Введите элементы через пробел", Top = 5, ForeColor = Color.Black, Font = new Font(FontFamily("Times New Roman"), 16.0f, FontStyle.Regular))
inp.Width <- 400
...
let ind13 =
    ...
    |> Observable.map(fun clickArgs -> let str = richTextBox1.Text
                                       ...
                                       richTextBox2.AppendText(g.ToString())
                                       ())
ind13
|> Observable.add(fun x -> let outp = new Label(Text = "Построенный список:", Top = 97, ForeColor = Color.Red, Font = new Font(FontFamily("Times New Roman"), 17.0f, FontStyle.Regular))
                           outp.Width <- 401
